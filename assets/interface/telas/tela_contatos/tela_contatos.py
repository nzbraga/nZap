...
def tela_contatos(raiz_principal):
    ...
    tk.Button(
        ...
        text="Excluir",
        font=("Arial", 10)
        , command=excluir_contato).pack(side=tk.LEFT, expand=True, padx=5)
...
ordenacao = {"NOME": False, "TELEFONE": False, "DATA": False}

def ordenar_por_coluna(col):
    global dados
    ordenacao[col] = not ordenacao[col]
    dados.sort(key=lambda x: x.get(col, ""), reverse=ordenacao[col])
    atualizar_lista()
...
def tela_contatos(raiz_principal):
    ...
    for col in columns:
        tree.heading(col, text=col, command=lambda c=col: ordenar_por_coluna(c) if c in ordenacao else None)
